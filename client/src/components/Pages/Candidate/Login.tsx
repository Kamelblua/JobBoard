...
const Login: FC<{}> = () => {
	...
	const onSubmit = (data: LoginData) => {
		...
		toast
			.promise(api.candidate.login(data), {
				pending: "Logging in...",
				success: {
					render() {
	...
	return (
		<Box className={styles.container}>
			<Helmet>
				<title>Job Board | Login as candidate</title>
			</Helmet>
			...
			<Box className={styles.flexContainer}>
				<Box className={styles.formContainer}>
					...
					<form className={styles.form} noValidate autoComplete='off' onSubmit={handleSubmit(onSubmit)}>
						...
						<Box className={styles.submitContainer}>
							<Link style={{ margin: "15px 0px" }} to='/register/candidate'>
								Not yet registered ?
							</Link>
				...
				<img className={styles.image} src='/assets/images/login-candidate.svg' alt='logo' />
			</Box>
		</Box>
	);
};
